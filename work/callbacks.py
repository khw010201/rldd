...
class TensorboardCallback(BaseCallback):
    def __init__(self, save_interval, save_path, verbose=1,
                 buffer_path="./metrics/callback_buffer.npz"):  # 추가
        ...
        try:
            ...
            self.lap_count_log = data['lap_count_log']
            self.collision_log = data['collision_log']
            self.episode_index = int(data['episode_index'])
            self.success_rate = float(data['success_rate'])
            self.collision_rate = float(data['collision_rate'])
            print(f"버퍼 로드 성공: success={self.success_rate:.3f}, collision={self.collision_rate:.3f}")
        except:
            self.lap_count_log = np.zeros(100, int)
            self.collision_log = np.zeros(100, int)
    ...
    def _on_step(self) -> bool:
        ...
        if 'episode' in infos:
            self.lap_count_log[self.episode_index] = int(infos["lap_count"][0])
            self.collision_log[self.episode_index] = int(infos['collision'])
            ...
            # 버퍼 저장
            np.savez(self.buffer_path,
                     lap_count_log=self.lap_count_log,
                     collision_log=self.collision_log,
                     episode_index=np.array(self.episode_index),
                     success_rate=np.array(self.success_rate),
                     collision_rate=np.array(self.collision_rate))
